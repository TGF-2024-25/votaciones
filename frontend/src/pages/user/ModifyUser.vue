<template>
  <div class="modify-user-container">
    ...
    <form @submit.prevent="submitForm">
      ...
      <div v-if="isAdmin" class="mb-3 d-flex flex-column">
...
export default {
  data() {
    return {
      ...
      errorMessage: "",
      isAdmin: false, // Determina si el usuario actual es administrador
    };
  },
  methods: {
    goBack() {
    ...
    checkAdmin() {
      ...
      if (token) {
        try {
          ...
          this.isAdmin = decoded.user?.type === "admin"; // Verificar si el usuario es administrador
        } catch (error) {
  ...
  mounted() {
    this.checkAdmin(); // Verificar si el usuario es administrador
